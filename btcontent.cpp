  ...
  if( Seeding() && (!m_flush_failed || IsFull()) ){
    if( !m_seed_timestamp ){
      ...
      if( Self.TotalDL() > 0 ){
        ...
        CONSOLE.Print("Total time used: %ld  s.",
          (long)((now - m_start_timestamp) ));
        //write total time used for download
        /*FILE * pfile;
        pfile = fopen("Stats.txt","a");
        fprintf(pfile, "Total time used: %ld mins\n", 
