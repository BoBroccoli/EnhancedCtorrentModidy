...
void Console::StatusLine0(char buffer[], size_t length)
{
  ...
  //wirte the peers count and download speed to the file;
  /*execution++;
  if(execution == 5){
  FILE * pfile;
  pfile = fopen("Stats.txt","a");
  fprintf(pfile, "Download rate: %d K/s\n", (int)(Self.RateDL() >> 10));
  fprintf(pfile, "Peers count: %d\n\n", (int)(Tracker.GetPeersCount()));
  fclose(pfile);
  execution++;
}*/
  if((int)(Self.RateDL() >> 10) > 0 && wroten == false){
      FILE * pfile;
      pfile = fopen("Stats.txt","a");
      fprintf(pfile, "Download rate: %d K/s\n", (int)(Self.RateDL() >> 10));
      fprintf(pfile, "Download start up after: %d s\n", (int)(now-BTCONTENT.GetStartTime()));
      fprintf(pfile, "Peers count: %d\n\n", (int)(Tracker.GetPeersCount()));
      fclose(pfile);
      wroten = true;
  }
  FILE * pfile;
  pfile = fopen("Stats.txt","a");
  fprintf((pfile), "Download time used: %d s\n", (int)(now-BTCONTENT.GetStartTime()));
  fclose(pfile);
  if((int)(now-BTCONTENT.GetStartTime()) == 19){
      FILE * pfile;
      pfile = fopen("Stats.txt","a");
      fprintf((pfile), "The torrent still not start download after 19s, torrent doesn't work!\n");
      fclose(pfile);
  }
